...
class MonitorProxy(multiprocessing.Process):
    ...
    def before_run(self):
        ...
        if CONFIG.svr_name == KaiwuDRLDefine.SERVER_LEARNER:
            self.prometheus_utils = PrometheusUtils(self.logger, False)
        elif CONFIG.svr_name == KaiwuDRLDefine.SERVER_AISRV or CONFIG.svr_name == KaiwuDRLDefine.SERVER_ACTOR:
            self.prometheus_utils = PrometheusUtils(self.logger, False)
        else:
            # 默认
            self.prometheus_utils = PrometheusUtils(self.logger, False)
    ...
    def run(self) -> None:
        ...
        while not self.exit_flag.value:
            try:
                ...
                self.logger.exception(f"monitor_proxy run error: {str(e)}")
