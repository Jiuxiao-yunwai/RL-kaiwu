...
Workflow: BFS Path Planning (DP approach)
工作流: BFS路径规划 (动态规划方法)

DP算法不需要训练循环，每一步直接根据观测计算最优动作。
但框架要求按照标准workflow接口运行游戏循环。
"""
...
from ..feature.definition import (
    sample_process,
    reward_shaping,
)
...
def workflow(envs, agents, logger=None, monitor=None):
    ...
    # 配置文件读取和校验
    usr_conf_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "conf", "train_env_conf.toml")
    usr_conf = read_usr_conf(usr_conf_path, logger)
    if usr_conf is None:
        logger.error(f"usr_conf is None, please check {usr_conf_path}")
        return
    ...
    if not valid:
        logger.error(f"check_usr_conf return False, please check {usr_conf_path}")
        return
    ...
    for epoch in range(epoch_num):
        ...
        # save model file (DP has no model, but framework requires this)
        # 保存model文件 (DP没有模型，但框架要求调用)
        now = time.time()
...
def run_episodes(n_episode, env, agent, g_data_truncat, usr_conf, logger, monitor, epoch):
    for episode in range(n_episode):
        ...
        while not done:
            # Agent performs inference (BFS path planning)
            # Agent 进行推理 (BFS路径规划)
            ...
            if truncated:
                logger.info(
                    ...
                    f"Collected:{collected_desc} | "
                    f"Bumps:{bump_cnt} | R:{episode_total_reward:.1f}"
                )
            elif terminated:
                logger.info(
